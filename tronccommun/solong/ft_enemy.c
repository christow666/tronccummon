...
/*   Updated: 2022/11/29 05:53:18 by cperron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_enemy_col(int x, int y, t_so_long *game)
{
	...
	while (i < game->x)
	{
		if (game->x_x[game->n_x] + x == game->x_x[i]
...
int	ft_enemy_move(int x, int y, int d, t_so_long *game)
{
	...
	if (game->map[game->x_y[game->n_x] + y][game->x_x[game->n_x] + x] == '1'
			|| game->map[game->x_y[game->n_x] + y][game->x_x[game->n_x] + x] == 'E')
		return (1);
...
{
	if (ft_enemy_move(game->x_dx[game->n_x], game->x_dy[game->n_x], 0, game) == 1)
	{
...
int	ft_enemy_ai_2(t_so_long *game)
{
	...
	game->x_dy[game->n_x] = 1;
		if (ft_enemy_move(game->x_dx[game->n_x], game->x_dy[game->n_x], 1, game) == 1)
		{
		game->x_dx[game->n_x] = -1;
		game->x_dy[game->n_x] = 0;
			if (ft_enemy_move(game->x_dx[game->n_x], game->x_dy[game->n_x], 2, game) == 1)
			{
				game->x_dx[game->n_x] = 0;
				game->x_dy[game->n_x] = -1;
			}
